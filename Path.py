...
class Path:
    ...
    def find_paths(self):
        ...
        if self.first_path is None:
            ...
            if set(self.first_path) == set(self.second_path):
                self.path_matrix = self.first_path
            else:
                self.path_matrix = (self.first_path, self.second_path)
    ...
    def visualize_path(self, path_color='white', alt_path_color='#F0E68C', marker_size=8):
        
        if isinstance(self.path_matrix, tuple):
            self.first_path, self.second_path = self.path_matrix

            x1_coords, y1_coords = zip(*self.first_path)
            x2_coords, y2_coords = zip(*self.second_path)

            plt.plot(y1_coords, x1_coords, marker='o', markersize=marker_size, color=path_color, label='Path1')
            plt.plot(y2_coords, x2_coords, marker='x', markersize=marker_size, color=alt_path_color, label='Path2')

        else:
            x_coords, y_coords = zip(*self.path_matrix)
            plt.plot(y_coords, x_coords, marker='o', markersize=marker_size, color=path_color, label='Path')
